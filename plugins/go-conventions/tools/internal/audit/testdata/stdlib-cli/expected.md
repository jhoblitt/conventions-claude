...
| --- | --- | --- | --- | --- | --- | --- |
| toolchain | go-directive | gap | tooling | go 1.26.1 | go 1.27, the minor only, and no toolchain line | set go 1.27 in go.mod and drop any toolchain line |
| lint | config | gap | tooling | no .golangci.yml | a .golangci.yml at the root, golangci-lint v2 (version: "2") | regenerate: goconv-audit --emit-golangci |
...
| makefile | present | gap | tooling | no Makefile | a Makefile at the repository root | copy templates/Makefile |
| makefile | targets | gap | tooling | no Makefile | every target templates/Makefile defines | copy templates/Makefile |
...
| deps | testing | gap | migration | missing: github.com/onsi/ginkgo/v2, github.com/onsi/gomega | Ginkgo v2 and Gomega in every _test.go | migrate area: testing |
...
| logging | stderr-json | skipped | none | log/slog is not imported | the JSON handler writes to stderr; stdout is program output |  |
| deps | flag | gap | migration | imported by: cmd/x/main.go | cobra, never the stdlib flag package | migrate area: cli |

24 gaps (18 tooling, 6 migration), 3 ok, 4 skipped
