...
# cache -- is AGENTS.md, Plugin tools.
# Unlike a hook launcher, which must get out of the way when it cannot build,
# every failure path here prints to stderr and exits non-zero.
#
...
# would silently pick author.name if the keys were reordered.
plugin=go-conventions
data="${XDG_CACHE_HOME:-$HOME/.cache}/conventions-claude/go-conventions"
...
# hooks/goconv-hook.sh will not do. -print0, sort -z and xargs -0 are GNU
