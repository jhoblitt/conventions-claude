// Package cli builds the {{BINARY}} command tree.
//
...
// flags, then {{ENV_PREFIX}}_* environment variables, then the config file,
...
func newRootCmd(stdin io.Reader, stdout, stderr io.Writer) *cobra.Command {
	...
	pf.String("log-level", "info", "log level: debug, info, warn, or error ({{ENV_PREFIX}}_LOG_LEVEL)")
	pf.String("log-format", "json", "log format: json or text ({{ENV_PREFIX}}_LOG_FORMAT)")
	cmd.Flags().String("name", "world", "who to greet ({{ENV_PREFIX}}_NAME)")
