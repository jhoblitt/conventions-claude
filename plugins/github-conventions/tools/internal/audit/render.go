...
func (r Report) Markdown() string {
	...
	b.WriteString("| Area | Check | Status | Current | Canon | Fix |\n")
	b.WriteString("| --- | --- | --- | --- | --- | --- |\n")
	for _, row := range r.Rows {
		fmt.Fprintf(&b, "| %s | %s | %s | %s | %s | %s |\n",
			cell(row.Area), cell(row.Check), cell(string(row.Status)),
...
func (r Report) summary() string {
	return fmt.Sprintf("%d gaps, %d ok, %d skipped", r.Gaps, r.OK, r.Skipped)
}
